    ...
    : QDialog(parent)
{
    ...
    if (existing)
    {
        ...
        if (existing->startDate.isValid())
            m_startDateEdit->setDate(existing->startDate);
...
ExpenseDialog::ExpenseDialog(QWidget *parent, const Expense *existing)
    : QDialog(parent)
{
    setWindowTitle(existing ? "Edit Expense" : "Add Expense");
    resize(420, 200);
    ...
    m_categoryEdit->setPlaceholderText("e.g., Art, Food, etc.");
    ...
    auto *buttons = new QDialogButtonBox(QDialogButtonBox::Ok | QDialogButtonBox::Cancel);
    connect(buttons, &QDialogButtonBox::accepted, this, &ExpenseDialog::accept);
    connect(buttons, &QDialogButtonBox::rejected, this, &ExpenseDialog::reject);
    ...
    if (existing)
    {
        ...
        m_amountSpin->setValue(existing->amount);
        m_categoryEdit->setText(existing->category);
    }
}

Expense ExpenseDialog::resultData() const
{
    Expense e;
    e.date = m_dateEdit->date();
    e.description = m_descriptionEdit->text().trimmed();
    e.amount = m_amountSpin->value();
    e.category = m_categoryEdit->text().trimmed();
    return e;
}
...
void ExpenseDialog::accept()
{
    ...
    {
        QMessageBox::warning(this, "Validation", "Please enter a description.");
        return;
    }
    if (m_amountSpin->value() < 0.0)
    {
...
void PaymentsDialog::editPayment()
{
    ...
    if (row < 0 || row >= m_workingCopy.payments.size())
        return;
